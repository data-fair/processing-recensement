    ...
    for (const code of codes) { // Pour chaque communes sélectionnées
      ...
      if (communes.length >= 1000) {
        ...
        try {
          ...
          if (error.response) {
            await log.error(`Error response: ${error.response.data}`)
          } else if (error.request) {
            await log.error('Request error:', error.request)
          } else {
            await log.error('General error:', error.message)
          }
          await log.error('Error config:', error.config)
        }
        communes = []
      }
    }
